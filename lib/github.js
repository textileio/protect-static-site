...
function serverHandler(req, res) {
    ...
    req.on('end', function (chunk) {
        ...
        if (self.secret) {
            var signature = req.headers['x-gitlab-token'];
    ...
    if (!req.headers.hasOwnProperty('x-gitlab-event')) {
        self.logger.error(Util.format('missing x-gitlab-event header from %s, returning 400', remoteAddress));
        failed = true;
        return reply(400, res);
    }
}
...
var GithubHook = function (options) {
    ...
    this.path = options.path || '/gitlab/callback';
...
GithubHook.prototype.listen = function (callback) {
    ...
    self.server.listen(self.port, self.host, function () {
        ...
        self.logger.log(Util.format('listening for gitlab events on %s:%d', self.host, self.port));
